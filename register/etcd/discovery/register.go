...
func (r *Register) register() error {
	...
	_, err = r.cli.Put(context.Background(), BuildRegPath(r.srvInfo), string(data), client3.WithLease(r.leasesID))
	return err
}
	...
	for {
		...
		case <-r.closeCh:
			...
			if err := r.unregister(); err != nil {
				logger.E("etct_register_keepAlive", "unregister failed err: %+v", err)
			}
			if _, err := r.cli.Revoke(context.Background(), r.leasesID); err != nil {
				logger.E("etct_register_keepAlive1", "revoke failed: %+v", err)
			}
			return
		case res := <-r.keepAliveCh:
			if res == nil {
				if err := r.register(); err != nil {
					logger.E("etct_register_keepAlive2", "register failed: %+v", err)
				}
			}
		case <-ticker.C:
			if r.keepAliveCh == nil {
				if err := r.register(); err != nil {
					logger.E("etct_register_keepAlive3", "register failed, err: %+v", err)
				}
			}
		}
	}
